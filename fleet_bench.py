...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--endpoints", nargs="*", default=None,
                   help="Multiple Ollama endpoints for fleet benchmarking. Overrides --host. "
                        "Example: http://192.168.1.6:11434 http://192.168.1.5:11434")
    p.add_argument("--concurrency-levels", nargs="*", type=int, default=None,
                   help="Run benchmark at multiple concurrency levels and compare. "
                        "Example: --concurrency-levels 1 4 8 15 30")
    p.add_argument("--endpoint-labels", nargs="*", default=None,
                   help="Short labels for each endpoint (same order as --endpoints). "
                        "Example: yuki kokkoro silvia kumo pecorine")
...
def create_tui_layout(metrics: LiveMetrics, benchmark_done: bool = False, menu_text: str = "", model_info: Optional[Dict[str, Dict[str, Any]]] = None, args: Optional[argparse.Namespace] = None, ep_labels: Optional[Dict[str, str]] = None) -> Layout:
    ...
    # Right column
    is_fleet = ep_labels is not None and len(ep_labels) > 1
    right_col = Text()
    right_col.append(f"Overall RPS: {snapshot['overall_rps']:.2f}\n", style="bold green")
    right_col.append(f"Recent RPS: {snapshot['recent_rps']:.2f}\n", style="green")

    # Fleet aggregate tok/s
    if is_fleet:
        fleet_tok = sum(
            stats.fmean(d["tokens_per_sec"]) if d["tokens_per_sec"] else 0
            for _, d in snapshot["per_model"].items()
        )
        right_col.append(f"Fleet tok/s: {fleet_tok:.1f}\n", style="bold yellow")

    # Tokens/sec if available
    if snapshot["tokens_per_sec"] > 0:
        right_col.append(f"Total Tokens: {snapshot['total_tokens']:,}\n", style="bold blue")
        right_col.append(f"Tok/s: {snapshot['tokens_per_sec']:.1f}\n", style="blue")
    else:
        if not is_fleet:
            right_col.append("\n\n")
...
class Bench:
    def __init__(self, args: argparse.Namespace):
        ...
        if args.endpoints:
            ...
            self.endpoints = [args.host]
        else:
            self.endpoints = ["http://localhost:11434"]
        # Create per-endpoint clients
        self.clients = {}
        for ep in self.endpoints:
            ck = {"host": ep, "timeout": args.timeout if args.timeout > 0 else None}
            if headers:
                ck["headers"] = headers
            self.clients[ep] = Client(**ck)
        # Endpoint labels for display
        if args.endpoint_labels:
            if len(args.endpoint_labels) != len(self.endpoints):
                raise SystemExit(f"--endpoint-labels count ({len(args.endpoint_labels)}) must match --endpoints count ({len(self.endpoints)})")
            self.ep_labels = dict(zip(self.endpoints, args.endpoint_labels))
        else:
            self.ep_labels = {ep: ep.replace("http://", "").replace("https://", "") for ep in self.endpoints}
    ...
    def _fetch_model_info(self, endpoint: str, model: str) -> Dict[str, Any]:
        """Fetch model information from Ollama API for a given endpoint"""
        ...
        try:
            ...
            self.model_info[key] = result
            return result
        except Exception:
            self.model_info[key] = {'parameter_size': '', 'quantization_level': '', 'family': ''}
            return self.model_info[key]
    ...
    def warmup(self) -> None:
        if self.args.warmup <= 0:
            return
        total_warmup = len(self.endpoints) * len(self.args.models) * self.args.warmup
        ...
        if self.args.tui and RICH_AVAILABLE:
            ...
            with Progress(
                ...
                task = progress.add_task(f"{len(self.endpoints)} endpoint(s) x {len(self.args.models)} model(s)", total=total_warmup)
                completed = 0
                for ep in self.endpoints:
                    ...
                    for m in self.args.models:
                        for i in range(self.args.warmup):
                            ...
                            try:
                                if self.args.chat:
                                    client.chat(model=m, messages=self._build_messages(prompt), options=self.options, stream=False)
                                else:
                                    client.generate(model=m, prompt=prompt, options=self.options, stream=False)
                                completed += 1
                                progress.update(task, completed=completed, description=f"({self.ep_labels[ep]}/{m})")
                            except Exception:
                                completed += 1
                                progress.update(task, completed=completed, description=f"({self.ep_labels[ep]}/{m}, error)")
        else:
            # Non-TUI mode
            if not self.args.silent:
                print(f"Warming up {len(self.endpoints)} endpoint(s) x {len(self.args.models)} model(s) with {self.args.warmup} requests each ({total_warmup} total)...")
            completed = 0
            for ep in self.endpoints:
                client = self.clients[ep]
                for m in self.args.models:
                    for i in range(self.args.warmup):
                        ...
                        try:
                            ...
                            completed += 1
                            if not self.args.silent:
                                print(f"  Warmup progress: {completed}/{total_warmup} ({self.ep_labels[ep]}/{m})", end="\r")
                        except Exception:
                            completed += 1
                            if not self.args.silent:
                                print(f"  Warmup progress: {completed}/{total_warmup} ({self.ep_labels[ep]}/{m}, error ignored)", end="\r")
            if not self.args.silent:
                print(f"  Warmup complete: {completed}/{total_warmup}          ")

    async def run(self, _inner: bool = False) -> Dict[str, Any]:
        # If concurrency-levels is set, run multiple benchmarks
        if self.args.concurrency_levels and not _inner:
            return await self._run_concurrency_levels()

        # Fetch model information before starting
        for ep in self.endpoints:
            for model in self.args.models:
                self._fetch_model_info(ep, model)

        self.warmup()
        tasks: List[Tuple[str, str, int]] = []  # (endpoint, model, idx)

        # Build tasks list - interleave across endpoints then models
        ...
        if self.live_metrics:
            ...
            for ep in self.endpoints:
                for m in self.args.models:
                    composite = f"{self.ep_labels[ep]}/{m}"
                    self.live_metrics.init_model(composite, self.args.requests)

        # Per-endpoint semaphores for fair distribution
        per_ep = max(1, self.args.concurrency // len(self.endpoints))
        ep_semaphores = {ep: asyncio.Semaphore(per_ep) for ep in self.endpoints}

        results: List[Dict[str, Any]] = []

        async def worker(endpoint: str, model: str, idx: int):
            nonlocal results
            async with ep_semaphores[endpoint]:
                if self.live_metrics and self.live_metrics.should_quit:
                    return

                result = await asyncio.to_thread(self._one_request, endpoint, model, idx)
                results.append(result)
                if not self.args.silent and not self.args.tui:
                    status = "OK" if result.get("error") is None else "ERR"
                    ep_label = self.ep_labels[endpoint]
                    print(f"[{status}] {ep_label}/{result['model']} req#{result['req_id']} duration={result['duration_ms']:.1f} ms" +
        ...
        if self.args.tui and self.live_metrics:
            ...
            # Run with TUI
            with Live(create_tui_layout(self.live_metrics, model_info=self.model_info, args=self.args, ep_labels=self.ep_labels), refresh_per_second=4, screen=True) as live:
                ...
                async def update_display():
                    while self.live_metrics and self.live_metrics.completed < self.live_metrics.total_tasks:
                        ...
                        if check_keyboard():  # Check for quit
                            break
                        live.update(create_tui_layout(self.live_metrics, model_info=self.model_info, args=self.args, ep_labels=self.ep_labels))
                    # Final update
                    if self.live_metrics:
                        live.update(create_tui_layout(self.live_metrics, benchmark_done=True, model_info=self.model_info, args=self.args, ep_labels=self.ep_labels))

                try:
                    await asyncio.gather(
                        asyncio.gather(*(worker(ep, m, i) for (ep, m, i) in tasks)),
                        update_display()
                    )
                finally:
            ...
            # Run without TUI
            await asyncio.gather(*(worker(ep, m, i) for (ep, m, i) in tasks))

        return self._summarize(results)

    def _one_request(self, endpoint: str, model: str, idx: int) -> Dict[str, Any]:
        prompt = apply_vars(self._pick_prompt(idx), self.variables)
        client = self.clients[endpoint]

        # Build composite model key for live metrics
        composite_model = f"{self.ep_labels[endpoint]}/{model}"

        # Notify live metrics that request is starting
        req_id = 0
        if self.live_metrics:
            req_id = self.live_metrics.start_request(composite_model, idx)
        ...
        try:
            if self.args.chat:
                if self.args.stream:
                    first = True
                    for chunk in client.chat(model=model, messages=self._build_messages(prompt), options=self.options, stream=True):
                        if first:
                    ...
                    resp = client.chat(model=model, messages=self._build_messages(prompt), options=self.options, stream=False)
                ...
                if self.args.stream:
                    first = True
                    for chunk in client.generate(model=model, prompt=prompt, options=self.options, stream=True):
                        if first:
                            ttft_ms = (time.perf_counter() - started) * 1000
                            first = False
                    ...
                    resp = client.generate(model=model, prompt=prompt, options=self.options, stream=False)
        ...
        return {
            "req_id": idx,
            "endpoint": self.ep_labels[endpoint],
    ...
    def _summarize(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        per_endpoint = {ep: summarize_group(rs) for ep, rs in by_endpoint.items()}
    ...
    async def _run_concurrency_levels(self) -> Dict[str, Any]:
        """Run benchmark at multiple concurrency levels and return comparison"""
        levels = sorted(set(self.args.concurrency_levels))
        results_by_level = {}

        # Save original concurrency
        orig_concurrency = self.args.concurrency

        for level in levels:
            if not self.args.silent:
                print(f"\n{'='*60}")
                print(f"  Concurrency level: {level} user(s)")
                print(f"{'='*60}")
            self.args.concurrency = level
            if self.live_metrics:
                self.live_metrics.reset_metrics()
            report = await self.run(_inner=True)
            results_by_level[level] = report["overall"]

        self.args.concurrency = orig_concurrency

        # Build comparison
        comparison = {
            "levels": levels,
            "per_level": results_by_level,
        }

        # Compute speedup relative to concurrency=1
        baseline_tok = None
        ...
        print("="*60)
        print(f"{'Users':>8} {'Req/s':>10} {'Lat avg':>10} {'Tok/s':>10} {'Speedup':>10}")
        print("-"*60)
        for level in levels:
            lvl = results_by_level[level]
...
def write_csv(path: str, rows: List[Dict[str, Any]]) -> None:
    keys = [
        "req_id","endpoint","model","chat","stream","prompt_chars","output_chars","prompt_eval_count","eval_count","started","ended","duration_ms","ttft_ms","error"
    ]
...
async def amain(argv: Optional[List[str]] = None):
    ...
    # If concurrency comparison, already printed; skip rest
    if report.get("concurrency_comparison"):
        return
    ...
    # Per-endpoint summary (fleet mode)
    if report.get("per_endpoint") and len(report["per_endpoint"]) > 1:
        print("\n--- Per-Endpoint ---")
        for ep, s in report["per_endpoint"].items():
            tp = f", throughput={s['throughput_rps']:.2f} rps" if s.get('throughput_rps') else ""
            print(f"\n- {ep}: {s['ok']}/{s['count']} ok, errors={s['errors']}{tp}")
